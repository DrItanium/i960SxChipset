...
public:
    template<byte numTagBits, typename T, byte totalBits, typename R>
    static void writeCacheLine(TaggedAddress<numTagBits, T, totalBits, R> address, const byte* buf) noexcept {
    ...
    template<byte numTagBits, typename T, byte totalBits, typename R>
    static void readCacheLine(TaggedAddress<numTagBits, T, totalBits, R> address, byte* buf) noexcept {
        // unlike a generic read/write operation, tagged addresses will never actually span multiple devices so there is no
        // need to do the offset calculation
        setChipId(address.getPSRAMChipId());
        SPI.beginTransaction(SPISettings(TargetBoard::runPSRAMAt(), MSBFIRST, SPI_MODE0));
